...
async function init(DOM) {
  ...
  const skyHDRTexture = await hdrLoader.loadAsync(
    "./SkyDome/blouberg_sunrise_2_1k.hdr"
  );
  ...
  gui.add(
    {
      toggleEnv: () => {
        skybox.material.uniforms.map.value =
          timer % 2 === 1 ? skytexture : skyHDRTexture;
        timer += 1;
        render();
      },
    },

...
/**
  ...
  每个卡片样式可以在外部使用 css 控制。
  
  注：对容器的响应式只针对 ui-component 元素里面自动创建的 canvas 响应，对外层无反应
...
/**
 * 设定 renderer
 * @param ref {DOM} 传入一个 DOM 元素作为容器，默认为空。当该参数为空时使用 window 作为容器。
 */
...
let requestAnimationFrameIndex = 0; // requestAnimationFrame 的函数 id，记住这个便于销毁
const render = () => {
